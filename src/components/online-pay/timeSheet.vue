...
<transition name="fade">
  ...
  <div v-if="showTag" class="main fixed bg_fff l0 r0 b0 overflow-y-auto overflow-x-hidden">
    <div class="flex fs20 color_000 mt50 ml30">
      <p class="mr20">{{dateObj.date}}</p>
      <p class="mr20">{{dateObj.weekNum}}</p>
      <p v-if="dateObj.ampm ==1">上午</p>
      <p v-if="dateObj.ampm ==2">下午</p>
    </div>
  ...
  * */
  ...
  export default {
      ...
      methods:{
        ...
        async saveTime(item){
          let arr = orderTimeCache.get();
          item.ampm = arr.ampm;
          await orderTimeCache.set(item);
        },
